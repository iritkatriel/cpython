...
#define except_star_block_type 1037
...
#define _loop0_68_type 1229
#define _loop1_69_type 1230
#define _loop1_70_type 1231
#define _tmp_71_type 1232
...
try_stmt_rule(Parser *p)
{
    ...
    { // 'try' ':' block finally_block
        ...
        )
        {
            ...
            int _end_col_offset = _token->end_col_offset;
            UNUSED(_end_col_offset); // Only used by EXTRA macro
            _res = _Py_Try ( b , NULL , NULL , f , 0 , EXTRA );
            if (_res == NULL && PyErr_Occurred()) {
                p->error_indicator = 1;
                D(p->level--);
                return NULL;
    ...
    { // 'try' ':' block except_block+ else_block? finally_block?
        ...
        )
        {
            ...
            int _end_col_offset = _token->end_col_offset;
            UNUSED(_end_col_offset); // Only used by EXTRA macro
            _res = _Py_Try ( b , ex , el , f , 0 , EXTRA );
            if (_res == NULL && PyErr_Occurred()) {
                p->error_indicator = 1;
                D(p->level--);
                return NULL;
    ...
    { // 'try' ':' block except_star_block+ else_block? finally_block?
        ...
        if (
            ...
            (b = block_rule(p))  // block
            &&
            (ex = (asdl_excepthandler_seq*)_loop1_49_rule(p))  // except_star_block+
            &&
            (el = else_block_rule(p), 1)  // else_block?
            &&
            (f = finally_block_rule(p), 1)  // finally_block?
        )
        {
            ...
            int _end_col_offset = _token->end_col_offset;
            UNUSED(_end_col_offset); // Only used by EXTRA macro
            _res = _Py_Try ( b , ex , el , f , 1 , EXTRA );
            if (_res == NULL && PyErr_Occurred()) {
                p->error_indicator = 1;
                D(p->level--);
                return NULL;
...
parameters_rule(Parser *p)
{
    ...
    { // slash_no_default param_no_default* param_with_default* star_etc?
        ...
        if (
            ...
            &&
            (b = (asdl_arg_seq*)_loop0_56_rule(p))  // param_no_default*
            &&
...
slash_no_default_rule(Parser *p)
{
    ...
    { // param_no_default+ '/' &')'
        ...
        asdl_arg_seq* a;
        if (
            (a = (asdl_arg_seq*)_loop1_63_rule(p))  // param_no_default+
            &&
            (_literal = _PyPegen_expect_token(p, 17))  // token='/'
            &&
...
decorators_rule(Parser *p)
{
    ...
    { // (('@' named_expression NEWLINE))+
        ...
        asdl_expr_seq* a;
        if (
            (a = (asdl_expr_seq*)_loop1_70_rule(p))  // (('@' named_expression NEWLINE))+
        )
        {
